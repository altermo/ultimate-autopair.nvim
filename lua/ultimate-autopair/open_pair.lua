local ranges_lib=require'ultimate-autopair.ranges'
---@param subranges ua.subranges
---@return fun(number,number):boolean
local function reverse_in_range_iter(subranges)
    local iter=ranges_lib.rev_iter_subranges(subranges)
    return coroutine.wrap(function(cols,cole)
        assert(cols<=cole)
        local next_start,next_end=iter()
        while true do
            local rcols,rcole=cols-1,cole-1
            while next_start and rcole<=next_start do
                next_start,next_end=iter()
            end
            if not next_start then
                break
            end
            local range_in_range=next_end>rcols
            local old_cols=cols
            cols,cole=coroutine.yield(not range_in_range)
            assert(cole<=old_cols)
            assert(cols<=cole)
        end
        while true do
...
---@param subranges ua.subranges
---@return fun(number,number):boolean
local function in_range_iter(subranges)
    local iter=ranges_lib.iter_subranges(subranges)
    return coroutine.wrap(function(cols,cole)
        assert(cols<=cole)
        local next_start,next_end=iter()
        while true do
            local rcols,rcole=cols-1,cole-1
            while next_start and rcols>=next_end do
                next_start,next_end=iter()
            end
            if not next_start then
                break
            end
            local range_in_range=next_start<rcole
            local old_cols=cols
            cols,cole=coroutine.yield(not range_in_range)
            assert(old_cols<=cols)
            assert(cols<=cole)
        end
        while true do
            coroutine.yield(true)
...
---Normally, it searches the range {row,col} to {-1,-1}(end of source)
---{gotostart_ret_pos} makes it search the range {0,0} to {row,col}
...
---@param exclude_ranges ua.ranges
...
function M.count_end_pair(
    ...
    for lrow,line in source.iter_lines(start_row,end_row) do
        local in_range
        if exclude_ranges[lrow]==true then
            goto continue
        elseif exclude_ranges[lrow] then
            local ranges=exclude_ranges[lrow] --[[@as ua.subranges]]
            in_range=reverse_in_range_iter(ranges)
        end
