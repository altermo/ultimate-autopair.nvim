...
function M.ext.fastwarp_over_pair(o,ind,p)
    ...
    return utils.delete(0,1)..utils.movel(matching_pair_pos-o.col-1)..p..utils.moveh(),matching_pair_pos-1
end
function M.ext.fastwarp_next_to_start_pair(o,ind,p)
    ...
    if not pair then return end
    if pair.rule and not pair.rule() then return end
    if not pair.fn.is_start(pair,o) then return end
...
function M.fastwarp_end(o,p,m)
    if o.col~=#o.line then
        return utils.delete(0,1)..'<end>'..p..utils.moveh(),#o.line-o.col-1
    end
